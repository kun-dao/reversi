"use strict";
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
var Generator = Object.getPrototypeOf(function () { return __generator(this, function (_a) {
    return [2 /*return*/];
}); });
Generator.prototype.map = function (mapper) {
    var _i, _a, val;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _i = 0, _a = this;
                _b.label = 1;
            case 1:
                if (!(_i < _a.length)) return [3 /*break*/, 4];
                val = _a[_i];
                return [4 /*yield*/, mapper(val)];
            case 2:
                _b.sent();
                _b.label = 3;
            case 3:
                _i++;
                return [3 /*break*/, 1];
            case 4: return [2 /*return*/];
        }
    });
};
Generator.prototype.filter = function (pred) {
    var _i, _a, val;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _i = 0, _a = this;
                _b.label = 1;
            case 1:
                if (!(_i < _a.length)) return [3 /*break*/, 4];
                val = _a[_i];
                if (!pred(val)) return [3 /*break*/, 3];
                return [4 /*yield*/, val];
            case 2:
                _b.sent();
                _b.label = 3;
            case 3:
                _i++;
                return [3 /*break*/, 1];
            case 4: return [2 /*return*/];
        }
    });
};
Generator.prototype.collect = function () {
    var result = [];
    for (var _i = 0, _a = this; _i < _a.length; _i++) {
        var val = _a[_i];
        result.push(val);
    }
    return result;
};
Generator.prototype.collectFor = function (forPred) {
    var result = [];
    var index = 0;
    for (var _i = 0, _a = this; _i < _a.length; _i++) {
        var val = _a[_i];
        if (!forPred(val, index++))
            break;
        result.push(val);
    }
    return result;
};
Generator.prototype.take = function (n) {
    var result = [];
    var index = 0;
    for (var _i = 0, _a = this; _i < _a.length; _i++) {
        var val = _a[_i];
        if (index++ >= n)
            break;
        result.push(val);
    }
    return result;
};
Generator.prototype.pick = function (pickPred) {
    var index = 0;
    for (var _i = 0, _a = this; _i < _a.length; _i++) {
        var val = _a[_i];
        if (pickPred(val, index++))
            return val;
    }
    return null;
};
Generator.prototype.first = function () {
    for (var _i = 0, _a = this; _i < _a.length; _i++) {
        var val = _a[_i];
        return val;
    }
    return null;
};
Generator.prototype.pickRelative = function (pickPred) {
    var _a;
    var _b = [this.next().value, this.next().value], x = _b[0], y = _b[1];
    while (true) {
        if (pickPred(x, y))
            return y;
        else
            _a = [y, this.next().value], x = _a[0], y = _a[1];
    }
};
exports.sequenceOf = function (f, init) {
    return (function () {
        var cur;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    cur = init;
                    _a.label = 1;
                case 1:
                    if (!true) return [3 /*break*/, 3];
                    return [4 /*yield*/, cur];
                case 2:
                    _a.sent();
                    cur = f(cur);
                    return [3 /*break*/, 1];
                case 3: return [2 /*return*/];
            }
        });
    })();
};
function foreverFrom(start) {
    var cur;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                cur = start;
                _a.label = 1;
            case 1:
                if (!true) return [3 /*break*/, 3];
                return [4 /*yield*/, cur++];
            case 2:
                _a.sent();
                return [3 /*break*/, 1];
            case 3: return [2 /*return*/];
        }
    });
}
exports.foreverFrom = foreverFrom;
function forever() {
    var cur;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                cur = 0;
                _a.label = 1;
            case 1:
                if (!true) return [3 /*break*/, 3];
                return [4 /*yield*/, cur++];
            case 2:
                _a.sent();
                return [3 /*break*/, 1];
            case 3: return [2 /*return*/];
        }
    });
}
exports.forever = forever;
function pairs(x, y) {
    var a, yrange, b;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                a = x[0];
                _a.label = 1;
            case 1:
                if (!(a <= x[1])) return [3 /*break*/, 6];
                yrange = y(a);
                b = yrange[0];
                _a.label = 2;
            case 2:
                if (!(b <= yrange[1])) return [3 /*break*/, 5];
                return [4 /*yield*/, { x: a, y: b }];
            case 3:
                _a.sent();
                _a.label = 4;
            case 4:
                b++;
                return [3 /*break*/, 2];
            case 5:
                a++;
                return [3 /*break*/, 1];
            case 6: return [2 /*return*/];
        }
    });
}
exports.pairs = pairs;
